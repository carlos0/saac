...
const getDatosSectores = async (req, res) => {
  ...
  try {
    ...
    console.log(`🚀 AREA_ZONA: Con_area ${mcUser.rows[0].con_area} - Con_zona ${mcUser.rows[0].con_zona}`);

    const con_zona_def = mcUser.rows[0].con_zona != 0 ? mcUser.rows[0].con_zona : mcUser.rows[0].con_area;
    const area_zona = mcUser.rows[0].con_zona != 0 ? 'con_zona' : 'con_area';
    ...
    ORDER BY sec_unico ASC`, [con_zona_def]);
